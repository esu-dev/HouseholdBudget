                                        ...
                                        <View key={extCat} style={{ marginBottom: 12, backgroundColor: colors.card, padding: 16, borderRadius: 16 }}>
                                            ...
                                            <ScrollView horizontal showsHorizontalScrollIndicator={false} style={{ flexDirection: 'row' }}>
                                                {CATEGORIES.flatMap(major => major.subCategories).map(minor => (
                                                    <TouchableOpacity 
                                                        key={minor.id}
                                                        onPress={() => setCategoryMappings(prev => ({ ...prev, [extCat]: minor.id }))}
                                                        style={{ 
                                                            paddingHorizontal: 12, 
                                                            paddingVertical: 8, 
                                                            borderRadius: 12, 
                                                            marginRight: 8,
                                                            backgroundColor: categoryMappings[extCat] === minor.id ? colors.indigo : colors.inputBg,
                                                            borderWidth: 1,
                                                            borderColor: categoryMappings[extCat] === minor.id ? colors.indigo : colors.border
                                                        }}
                                                    >
                                                        <Text style={{ fontSize: 12, color: categoryMappings[extCat] === minor.id ? 'white' : colors.text, fontWeight: categoryMappings[extCat] === minor.id ? 'bold' : 'normal' }}>
                                                            {minor.label}
                                                        </Text>
                                                    </TouchableOpacity>
                                                ))}
                                        ...
                                        <View key={extAcc} style={{ marginBottom: 12, backgroundColor: colors.card, padding: 16, borderRadius: 16 }}>
                                            ...
                                            <ScrollView horizontal showsHorizontalScrollIndicator={false} style={{ flexDirection: 'row' }}>
                                                {accounts.map(acc => (
                                                    <TouchableOpacity 
                                                        key={acc.id}
                                                        onPress={() => setAccountMappings(prev => ({ ...prev, [extAcc]: acc.id }))}
                                                        style={{ 
                                                            paddingHorizontal: 12, 
                                                            paddingVertical: 8, 
                                                            borderRadius: 12, 
                                                            marginRight: 8,
                                                            backgroundColor: accountMappings[extAcc] === acc.id ? colors.indigo : colors.inputBg,
                                                            borderWidth: 1,
                                                            borderColor: accountMappings[extAcc] === acc.id ? colors.indigo : colors.border
                                                        }}
                                                    >
                                                        <Text style={{ fontSize: 12, color: accountMappings[extAcc] === acc.id ? 'white' : colors.text, fontWeight: accountMappings[extAcc] === acc.id ? 'bold' : 'normal' }}>
                                                            {acc.name}
                                                        </Text>
                                                    </TouchableOpacity>
                                                ))}
