...
import { ArrowDownCircle, ArrowUpCircle, Calendar, ChevronLeft, CircleEllipsis, ExternalLink, FileUp, Store, Trash2, Wallet } from 'lucide-react-native';
import React, { useEffect, useRef, useState } from 'react';
import { Controller, useForm } from 'react-hook-form';
import { Alert, InputAccessoryView, Keyboard, KeyboardAvoidingView, Linking, Platform, ScrollView, Text, TextInput, TouchableOpacity, useColorScheme, View } from 'react-native';
...
export default function InputScreen() {
  ...
  const { addTransaction, addTransactions, updateTransaction, deleteTransaction, accounts, fetchData, editingTransaction, setEditingTransaction, majorCategories } = useTransactionStore();
  ...
  const navigateAfterAction = () => {
    ...
    // 編集モードだった場合は、前の画面（口座履歴等）に戻ることを最優先にする
    // navigation.canGoBack() を使用することで、タブの外側のスタックも考慮できる場合がある
    if (wasEditing) {
    ...
    <View style={{ flex: 1, backgroundColor: colors.background }}>
      ...
      >
        ...
        <View style={{ backgroundColor: colors.card, padding: 24, borderRadius: 24, marginBottom: 24, shadowColor: '#000', shadowOffset: { width: 0, height: 1 }, shadowOpacity: 0.1, shadowRadius: 2, elevation: 2 }}>
            ...
            {errors.category_id && (
              <Text style={{ color: colors.danger, fontSize: 14, marginTop: 4 }}>{errors.category_id.message}</Text>
            )}

            <Text style={{ color: colors.textMuted, marginTop: 16, marginBottom: 12, fontWeight: '500' }}>アカウント</Text>
            <View style={{ flexDirection: 'row', flexWrap: 'wrap' }}>
      ...
      <View style={{ padding: 16, backgroundColor: colors.card, borderTopWidth: 1, borderTopColor: colors.border, paddingBottom: 32 }}>
        <TouchableOpacity 
          ...
          style={{ 
            ...
            backgroundColor: transactionType === 'expense' ? colors.primary : colors.income 
          }}
        >
